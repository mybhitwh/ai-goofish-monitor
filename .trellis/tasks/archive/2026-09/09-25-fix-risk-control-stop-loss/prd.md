...
1. **范围**：本期做 R1–R5（P0+P1）。理由是 R4/R5 与本次事故同源且改动极小，R6/R7 涉及调度节奏与请求量的产品取舍，已定另立任务（2026-09-26）。
...
- **本期范围止于 R1–R5（P0+P1）；R6（错峰与并发上限）与 R7（降请求量 / 浏览器上下文复用）另立任务**，不在本任务实现。理由：R1 修复后组内任务不再被单任务 job 重复调度，"同分钟拉起 4 个进程"的主因已消失，R6 的主要诉求被覆盖；R6/R7 属采集节奏的产品取舍，塞进事故补丁会让验证面与回归风险翻倍。
- R6/R7 的残留暴露（手动触发与组循环并存、跨组同分钟启动）在另立任务里评估；最小防线是"同一时刻只允许一个爬虫进程"的入口锁，作为该任务的第一条。