const zhCNExtra = {
  tasks: {
    ...
    groups: {
      title: '任务组管理',
      description: '把多个任务编入一组，统一配置调度时间；组内任务可串行或并行执行。',
      manageButton: '任务组',
      createTitle: '新建任务组',
      editTitle: '编辑任务组',
      createAction: '新建任务组',
      name: '组名称',
      namePlaceholder: '例如：iPad Air 监控组',
      cron: '统一调度（Cron）',
      cronPlaceholder: '例如：0 9,21 * * *',
      cronHint: '组内任务在此时间统一触发；留空则不自动调度。',
      executionMode: '组内执行方式',
      modeSerial: '串行执行',
      modeSerialHint: '组内任务依次执行：上一个完成后再启动下一个，对风控更友好。',
      modeParallel: '并行执行',
      modeParallelHint: '组内任务同时启动，速度更快但请求更密集。',
      enabled: '启用任务组',
      noCron: '未设置定时',
      memberCount: '{count} 个任务',
      membersHint: '勾选任务加入本组，取消勾选移出（一个任务只属一个组）：',
      memberRunning: '任务运行中，请先停止再调整分组',
      inOtherGroup: '已在其他组',
      noTasksAvailable: '暂无任务，先创建任务后再来分组',
      startNow: '立即执行',
      empty: '暂无任务组',
      deleteConfirm: '确定删除任务组「{name}」吗？组内 {count} 个任务将恢复独立调度。',
      validation: { nameRequired: '组名称不能为空' },
      toasts: {
        created: '任务组创建成功',
        updated: '任务组已更新',
        saveFailed: '保存任务组失败',
        deleted: '任务组已删除',
        deleteFailed: '删除任务组失败',
        started: '任务组已触发执行',
        startFailed: '触发任务组失败',
        stopped: '任务组内任务已停止',
        stopFailed: '停止任务组失败',
        memberAdded: '任务已加入任务组',
        memberRemoved: '任务已移出任务组',
      },
    },
    generation: {
      title: '任务生成进度',
    ...
    form: {
      ...
      group: '所属任务组',
      groupNone: '不属于任何任务组',
      groupNoneHint: '未分组任务使用自身的定时规则。',
      groupHint: '已加入任务组，定时规则由任务组统一管理（当前：{cron}）。',
      groupNoCron: '任务组未设置定时',
      accountStrategy: {
        auto: '自动选择',
